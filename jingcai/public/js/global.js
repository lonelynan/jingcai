...
{	   	
	var upurl ='/index.php?m=Home&c=UploadifyController.class&a=upload&num='+num+'&input='+elementid+'&path='+path+'&func='+callback;
    layer.open({
        type: 2,
        title: '上传图片',
        shadeClose: true,
...
function ClearPicArr(val)
{
	$("li[rel='"+ val +"']").remove();
	$.get(
		"{:U('Admin/UploadifyController.class/delupload')}",{action:"del", filename:val},function(){}
	);
}
/*
 * 删除组图input
...
function ClearPicArr2(val)
{
    $("li[rel='"+ val +"']").remove();
    $.get(
        "{:U('Home/UploadifyController.class/delupload')}",{action:"del", filename:val},function(){}
    );
}
