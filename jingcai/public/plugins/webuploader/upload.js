$(function() {
	...
	function webuploader(opts){
		...
		uploader.on( 'uploadSuccess', function( file,data) {
			...
			if(data.state == 'SUCCESS'){
				...
				sLi += '<img src="' + data.url + '" width="100" height="100" onerror="this.src=Uploadify">';
				sLi += '<input type="hidden" Uploadifype="hidden" name="fileurl_tmp[]" value="' + data.url + '">';
			...
			if ( file.getStatus() === 'invalid' ) {
				...
				uploader.makeThumb( file, function( error, src ) {
					...
					if( isSupportBase64 ) {
						...
						$.ajax("/index.php?m=Admin&c=UploadifyController.class&a=preview", {
							method: 'POST',
							data: src,
							dataType:'json'
	...
	window.Manager = {
		...
		sure : function($file){
			...
			sLi += '<input type="hidden" Uploadifytype="hidden" name="fileurl_tmp[]" value="' + data.url + '">';
			sLi += '<a href="javascript:void(0);">删除</a>';
			sLi += '</li>';
			$(".fileWarp ul").append(sLi);
		...
		del:function(obj){
			if(!confirm('确定删除吗?')) return false;
		    $.get("/index.php?m="+moudle+"&c=UploadifyController.class&a=delupload",{action:"del", filename:$(obj).attr('data-img')},function(res){
					if(res == 1){
