<?php
namespace app\mobile\controller\base;
...
class Base extends Controller {
    ...
    public function _initialize() {
        ...
        // 微信Jssdk 操作类
        $jssdk = new JssdkLogic($this->weixin_config['appid'], $this->weixin_config['appsecret']);
...
//        Session::start();
        ...
        $wx_qr = db('wx_user')->cache(true)->value('qr'); //获取微信配置
       ...
       /* if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
            ...
            if (isset($user_temp['user_id']) && $user_temp['user_id']) {
                $user = db('users')->where("id", $user_temp['user_id'])->find();
                if (!$user) {
                    $_SESSION['openid'] = 0;
                    session('user', null);
                }
            ...
            if (empty($_SESSION['openid'])) {
                $this->weixin_config = db('wx_user')->find(); //获取微信配置
                ...
                if(is_array($this->weixin_config) && $this->weixin_config['wait_access'] == 1){
                    ...
                    if($is_bind_account){
                        if($wxuser['unionid']){
                            $thirdUser = db('OauthUsers')->where(['unionid'=>$wxuser['unionid'], 'oauth'=>$wxuser['oauth']])->find();
                        }else{
                            $thirdUser = db('OauthUsers')->where(['openid'=>$wxuser['openid'], 'oauth'=>$wxuser['oauth']])->find();
                        }
                        if(empty($thirdUser)){
                            //用户未关联账号, 跳到关联账号页
                            session('third_oauth',$wxuser);
                            $first_leader = input('first_leader');
                            return $this->redirect(url('Mobile/User/bind_guide',['first_leader'=>$first_leader]));
                        }else{
                            //微信自动登录
    ...
    public function public_assign()
    {
       ...
       $tp_config = db('config')->cache(true,TPSHOP_CACHE_TIME)->select();
    ...
    public function get_access_token(){
        ...
        db('wx_user')->where(array('id'=>$this->weixin_config['id']))->update(array('web_access_token'=>$return['access_token'],'web_expires'=>$web_expires));
