...
class LoginApi extends Base {
    ...
    public function __construct(){
        ...
        //获取配置
        $data = db('Plugin')->where("code=:code and  type = 'login' ")->bind(['code'=>$this->oauth])->find();
    ...
    public function callback(){
        ...
        if($is_bind_account){
            ...
            if($data['unionid']){
                $thirdUser = db('OauthUsers')->where(['unionid'=>$data['unionid'], 'oauth'=>$data['oauth']])->find();
            }else{
                $thirdUser = db('OauthUsers')->where(['openid'=>$data['openid'], 'oauth'=>$data['oauth']])->find();
            }
            ...
            if(empty($thirdUser)){
                ...
                return $this->redirect(url('Mobile/User/bind_guide'));
            }else{
                //微信自动登录
        ...
        if($data['status'] == 1){
            ...
            db('cart')->where("session_id" ,$this->session_id)->save(array('user_id'=>$data['result']['user_id']));
