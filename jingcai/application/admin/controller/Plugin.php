...
class Plugin extends Base {
    ...
    public function install(){
        ...
        if($condition['type'] == 'payment' && $update['status']){
            ...
            $add['config_value'] = '';
            $model->where($condition)->insert($add);
        }
     ...
     * 插件信息配置
     */
    public function setting($type='',$code=''){

        $condition['type'] = $type;
        $condition['code'] = $code;
        $model = db('plugin');
        if(($condition["code"] == "unionpay")){ header("Content-type: text/html; charset=utf-8"); }
        if($condition["type"] == "login"  && $condition["code"] == "weixin"){ header("Content-type: text/html; charset=utf-8"); }
