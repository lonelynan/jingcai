...
class Article extends Base {
    ...
    public function article($act='',$article_id='' ){
        $ArticleCat = new ArticleCatLogic();
            //$act = input('GET.act','add');
            $info = array();
    ...
    public function categoryHandle()
    {
        ...
        if ($data['act'] == 'add') {
            $r = db('article_cat')->insert($data);
        } elseif ($data['act'] == 'edit') {
        	$cat_info = db('article_cat')->where("cat_id",$data['cat_id'])->find();
        	if($cat_info['cat_type'] == 1 && $data['parent_id'] > 1){
        		$this->ajaxReturn(['status' => -1, 'msg' => '可更改系统预定义分类的上级分类']);
        	}
        	$r = db('article_cat')->where("cat_id",$data['cat_id'])->update($data);
        } elseif ($data['act'] == 'del') {
        	if($data['cat_id']<9){
        		$this->ajaxReturn(['status' => -1, 'msg' => '系统默认分类不得删除']);
        	}
        	if (db('article_cat')->where('parent_id', $data['cat_id'])->count()>0)
        	{
        		$this->ajaxReturn(['status' => -1, 'msg' => '还有子分类，不能删除']);
        	}
        	if (db('article')->where('cat_id', $data['cat_id'])->count()>0)
        	{
    ...
    public function aticleHandle()
    {
        ...
        //$this->ajaxReturn(['status' => 0, 'msg' =>$result, 'result' => $result]);
        if ($result !== true) {
            $this->ajaxReturn(['status' => 0, 'msg' => '参数错误', 'result' => $result]);
        }
        ...
        if ($data['act'] == 'add') {
        	...
        	$data['add_time'] = time(); 
            $r = db('article')->insert($data);
        } elseif ($data['act'] == 'edit') {

            $r = db('article')->where('article_id='.$data['article_id'])->data($data)->update();

        } elseif ($data['act'] == 'del') {
        	$r = db('article')->where('article_id='.$data['article_id'])->delete(); 	
        }
    ...
    public function linkHandle(){
        ...
        if($data['act'] == 'add'){
            $r = db('friend_link')->insert($data);
        }
        if($data['act'] == 'edit'){
            $r = db('friend_link')->where('link_id='.$data['link_id'])->save($data);
        }
    	if($r){
