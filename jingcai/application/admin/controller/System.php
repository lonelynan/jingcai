...
//            'smtp'      => '邮件设置'
		...
		$inc_type =  input('inc_type','system_info');
		$this->assign('inc_type',$inc_type);
		$config = tpCache($inc_type);
		if($inc_type == 'shop_info'){
			$province = db('region')->where(array('parent_id'=>0))->select();
			$city =  db('region')->where(array('parent_id'=>$config['province']))->select();
			$area =  db('region')->where(array('parent_id'=>$config['city']))->select();
    ...
    public function addEditNav()
    {
        ...
        if (IS_POST) {
            if (input('id')){
                $model->update(input('post.'));
            }else{
                $model->insert(input('post.'));
            }
    ...
    {
        // 删除导航
        db('Navigation')->where("id",input('id'))->delete();
        $this->success("操作成功!!!",Url('Admin/System/navigationList'));
    }

	public function refreshMenu(){
		$pmenu = $arr = array();
		$rs = db('system_module')->where('level>1 AND visible=1')->order('mod_id ASC')->select();
		foreach($rs as $row){
			if($row['level'] == 2){
        ...
        public function cleanCache(){
			...
			Cache::clear();
            $script = "<script>parent.layer.msg('缓存清除成功', {time:3000,icon: 1});window.location=Admin;</sAdmin>";
            echo $script;
            exit;
        }
     ...
     */
     public function login_task()
     {
        ...
        $order_id_arr = db('order')->where("order_status = 1 and shipping_status = 1 and shipping_time < $time")->getField('order_id',true);
     ...
     function right_list(){
     	$group = array('system'=>'系统设置','content'=>'内容管理','member'=>'会员中心',
     			'marketing'=>'营销推广','tools'=>'插件工具','count'=>'统计报表'
     	);

     	$name = input('name');
     	if($name){
     	    $condition['name|right'] = array('like',"%$name%");
     	    $right_list = db('system_menu')->where($condition)->order('id desc')->select();
     	}else{
     	    $right_list = db('system_menu')->order('id desc')->select();
     	}
     ...
     public function edit_right(){
     	if(IS_POST){
     		...
     		$this->success('操作成功',Url('System/right_list'));
     		exit;
     	}
     	$id = input('id');
     	if($id){
     	...
     	$group = array('system'=>'系统设置','content'=>'内容管理','member'=>'会员中心','tools'=>'插件工具'
     	);
