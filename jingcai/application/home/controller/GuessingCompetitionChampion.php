...
class GuessingCompetitionChampion extends Base
{
    ...
    public function index()
    {
        ...
        $activity_settings = db('activity_settings_champion')->alias('asc')
            ...
            ->field('at1.name name1,at1.activity_team_image activity_team_image1,at2.name name2,at2.activity_team_image activity_team_image2,at3.name name3,at3.activity_team_image activity_team_image3,at4.name name4,at4.activity_team_image activity_team_image4,at1.status as status1,at2.status as status2,at3.status as status3,at4.status as status4,asc.activity_settings_champion_id
            ,asc.activity_team1_id ,asc.activity_team2_id,asc.activity_team3_id ,asc.activity_team4_id,asc.title')
        ...
        if(!empty(session('user'))){
            ...
            if($activity_team_id){
                ...
                if(db('activity_comp_result_champion')->where(['user_id'=>session('user')['id']])->count()>0){
                    $this->ajaxReturn(['status'=>0,'msg'=>'您已猜过冠军','result'=>'']);
                }
