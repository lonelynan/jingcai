...
class SmsLogic 
{
    ...
    {
        $smsTemp = db('sms_template')->where("send_scene", $scene)->find();    //用户注册.
        ...
        //发送记录存储数据库
        $log_id = db('sms_log')->insertGetId(array('mobile' => $sender, 'code' => $code, 'add_time' => time(), 'session_id' => $session_id, 'status' => 0, 'scene' => $scene, 'msg' => $msg));
        if ($sender != '' && check_mobile($sender)) {//如果是正常的手机号码才发送
            ...
            if ($resp['status'] == 1) {
                db('sms_log')->where(array('id' => $log_id))->update(array('status' => 1)); //修改发送状态为成功
            }else{
                db('sms_log')->where(array('id' => $log_id))->update(array('error_msg'=>$resp['msg'])); //发送失败, 将发送失败信息保存数据库
            }
            return $resp;
        } else {
...
$type=2;
        switch($type) {
            ...
            case 1:
                ...
                break;
            case 2:
                $result = $this->dsfSms($mobile, $smsSign, $smsParam, $templateCode);
                break;
            default:
    ...
    private function dsfSms($mobile, $smsSign, $smsParam, $templateCode){

        $sendurl="http://h.1069106.com:1210/services/msgsend.asmx/SendMsg?userCode=WGZG&userPass=WGZG123&DesNo=".$mobile."&Msg=验证码：".json_decode($smsParam,true)['code']."。请不要把验证码泄露给其他人。如非本人操作，可不用理会。【威固中国】&Channel=1";
